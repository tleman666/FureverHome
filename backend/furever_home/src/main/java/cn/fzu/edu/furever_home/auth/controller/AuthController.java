...
public class AuthController {
    ...
    public Result<?> login(@RequestBody @Valid LoginRequest req) {
        User u = authService.findByAccount(req.getAccount());
        if (u == null)
            return Result.fail(401, "账户不存在");
    ...
    public Result<?> sendCode(@RequestBody @Valid SendCodeRequest req) {
    ...
    public Result<?> confirm(@RequestBody @Valid ConfirmRegisterRequest req) {
    ...
    public Result<?> sendResetCode(@RequestBody @Valid ResetPasswordSendCodeRequest req) {
    ...
    public Result<?> resetPassword(@RequestBody @Valid ResetPasswordRequest req) {
